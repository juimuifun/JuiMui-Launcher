...
class Settings {
    ...
    async javaPath() {
        ...
        let configClient = await this.db.readData('configClient')
        let javaPath = configClient?.java_config?.java_path || 'ใช้เวอร์ชันของ Java ที่มาพร้อมกับ Launcher';
        ...
        document.querySelector(".java-path-set").addEventListener("click", async () => {
            ...
            } else alert("ชื่อไฟล์ต้องเป็น java หรือ javaw");
        });

        document.querySelector(".java-path-reset").addEventListener("click", async () => {
            let configClient = await this.db.readData('configClient')
            javaPathInputTxt.value = 'ใช้เวอร์ชันของ Java ที่มาพร้อมกับ Launcher';
